...
(function () {
    // Toggle for per-plot image gallery rendering
    const SHOW_PLOT_GALLERY = false;
    ...
    function renderFastqcSectionFactory(API_BASE) {
      return function renderFastqcSection(container, artifacts, tag, title) {
        // Disabled: do not render per-plot image gallery
        if (!SHOW_PLOT_GALLERY) return;
        const plots = (artifacts || [])
          .filter(a => a.kind && a.kind.startsWith(`fastqc_plot_${tag}:`))
          .map(a => {
            const parts = a.kind.split(":");
            const key = parts.pop() || "plot";
            const mate = parts.length > 1 ? parts[1] : null;
            const mateLabel = mate ? `[${mate.toUpperCase().replace(/_/g, " ")}] ` : "";
            return {
              src: makeArtifactUrl(API_BASE, a.path),
              label: `${mateLabel}${PRETTY[key] || key}`,
            };
          });
