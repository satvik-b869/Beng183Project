...
async function pollStatus() {
  ...
  try {
    ...
    function addQcFrame(title, stage){
      if (!stage || !stage.artifact) return;
      const url = toQcUrl(job.id, stage.artifact);
      if (!url) return;
      ...
      if (window.PlotUI && stage.metrics) {
        window.PlotUI.renderMetrics(statusRoot, `${title} Metrics`, stage.metrics);
      }
    }
